...
import cn.com.fos.xerox.scanner.XMain;
...
public abstract class Scanner {
	...
	protected void progress(int progress, boolean last) {
		...
		if (np > precent || last) {
			precent = last ? 100 : np;
			if (XMain.progress != null) {
				XMain.progress.setCurrent(precent);
				return;
			}
			char[] indicator = StringUtils.leftPad("", 76, '-').toCharArray();
			int pos = 76 * precent / 100;
			if (pos < indicator.length) indicator[pos] = '$';
			System.arraycopy(String.format("%3d%%", precent).toCharArray(), 0, indicator, 36, 4);
			System.err.print(isTerm() ? "\033[80D\033[2K[" + new String(indicator) + "]" : "");
			if (last || !isTerm())
				System.err.println("");
		}
	}
	 ...
	 * start scan
	 * 
	 * @see cn.com.fos.xerox.scanner.driver.Scanner#scan()
	 */
